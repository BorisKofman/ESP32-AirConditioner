#pragma once
// Replaces the compiled-in CHIP test commissioning parameters (passcode
// 20202021 / discriminator 3840, shared by every Arduino Matter example)
// with per-device values from Config.h.
//
// Must be applied AFTER Matter.begin(): the esp32 core is built with
// CONFIG_EXAMPLE_COMMISSIONABLE_DATA_PROVIDER=y, so esp_matter's startup
// installs the example provider (test passcode) over anything set earlier.
...
// VID/PID must match the ones compiled into the esp32 core's Basic
// Information cluster (CONFIG_DEVICE_VENDOR_ID / CONFIG_DEVICE_PRODUCT_ID)
// so the QR payload agrees with what the device reports after pairing.
...
public:
  ...
  // The verifier is derived from the passcode at runtime, so no offline
  // spake2p tooling is needed when changing the passcode in Config.h.
...
// Install our provider (last writer wins over the example provider) and, if
// the commissioning window is already open (uncommissioned device), reopen it
// so it advertises and verifies with our discriminator/passcode instead of
// the test values it captured at startup.
